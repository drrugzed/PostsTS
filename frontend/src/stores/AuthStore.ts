...
class AuthStore {
  ...
  register = async (registerData: IRegisterFormData) => { // принимает данные с формы и использует интерфейс регистрации
    ...
    try { 
      ...
      if (axios.isAxiosError(error)) { // проверяем ошибка ли это запроса
  ...
  login = async (loginFormData: ILoginFormData) => { // принимает данные с формы 
    this.isLoading = true; 
    try {
